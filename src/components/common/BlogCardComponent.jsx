...
const BlogCard = ({ 
  ...
  return (
    <article className={` h-full w-[270px] lg:w-[300px] 2xl:w-[350px] flex flex-col gap-[8px] pb-[20px] bg-white rounded-[6px] overflow-hidden ${className}`}>
      <div className="p-4 flex flex-col gap-[8px]">
        <ImageContainer src={imageSrc} alt={imageAlt} />
        <CardTitle>{title}</CardTitle>
        <CardDescription>{description}</CardDescription>
        <div className="mt-4">
           <Button className={'w-full hover:border-none bg-transparent border border-text-color2'} lable={'Read More'} />
        </div>
      </div>
    </article>
  );
